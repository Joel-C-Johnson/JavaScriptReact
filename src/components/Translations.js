...
class Translations extends Component {
	...
	async getVerseText(token) {
		...
		const data = await fetch(
			'http://127.0.0.1:8000/v1/concordances/' + language + '/' + book + '/' + this.state.selectedToken,
			{
				method: 'GET',
			}
		);
	...
	getTokens() {
		...
		if (this.props.data.book) {
			return this.props.data.tokenList.map(item => {
				return (
					<div key={item}>
						<ListItem button key={item} name={item} value={item} onClick={this.handleClick}>
							{item}
						</ListItem>
						<Divider />
					</div>
			...
			return (
				<ListItem key="" button>
					Select Version, Book to load Tokens <Divider />
				</ListItem>
			);
		} else {
			return (
				<ListItem key="" button>
					Select Language, Version, Book to load Tokens <Divider />
				</ListItem>
			);
		}
	}
